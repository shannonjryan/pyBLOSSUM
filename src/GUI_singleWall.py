...
class PackageLoader(QThread):
    ...
    def run(self):
        ...
        from BLE_singleWall import singleWall_performance
        ...
        self.packages_loaded.emit({
            ...
            'singleWall_performance': singleWall_performance
        })
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.initial_target_type = "Aluminium"
        ...
        self.target_type_label = QLabel("Material type:", self.frame2)
        ...
        self.target_type_dropdown.addItems(["Aluminium", "Steel", "Titanium", "CFRP", "Fibreglass"])
        ...
        # Create a dropdown to select the specific target material
        self.material1_label = QLabel("Material:", self.frame2)
        self.frame2_layout.addWidget(self.material1_label, 1, 0)
        ...
        self.frame2_layout.addWidget(self.material1_dropdown, 1, 1)
        ...
        self.frame2_layout.addWidget(self.density1_label, 3, 0)
        self.density1_entry = QLineEdit(self)
        self.density1_entry.setFixedWidth(widget_width)        
        self.frame2_layout.addWidget(self.density1_entry, 3, 1)
        ...
        self.hardness_label = QLabel("Hardness (HB):", self)
        self.frame2_layout.addWidget(self.hardness_label, 4, 0)
        self.hardness_entry = QLineEdit(self)
        self.hardness_entry.setFixedWidth(widget_width)    
        self.frame2_layout.addWidget(self.hardness_entry, 4, 1)   
        # self.hardness_entry.setText(str(df_materials.loc[df_materials['mat'] == self.initial_target_mat, 'hardness'].values[0]))
        self.hardness_entry.setText("95")

        self.soundspeed_label = QLabel("Soundspeed (km/s):", self)
        self.frame2_layout.addWidget(self.soundspeed_label, 5, 0)
        self.soundspeed_entry = QLineEdit(self)
        self.soundspeed_entry.setFixedWidth(widget_width)             
        self.frame2_layout.addWidget(self.soundspeed_entry, 5, 1)
        # soundspeed = np.sqrt(df_materials.loc[df_materials['mat'] == self.initial_target_mat, 'tensile_modulus'].values[0]/df_materials.loc[df_materials['mat'] == self.initial_target_mat, 'density'].values[0])
        # soundspeed = self.packages['np'].sqrt(68.9E9/2700)
        soundspeed = sqrt(68.9E9/2700)/1000  # units = km/s
        self.soundspeed_entry.setText(f'{soundspeed:.2f}')           

        self.MLI_label = QLabel("MLI AD (g/cm²):", self)
        self.frame2_layout.addWidget(self.MLI_label, 6, 0)
        self.MLI_entry = QLineEdit(self)
        self.MLI_entry.setFixedWidth(widget_width)             
        self.frame2_layout.addWidget(self.MLI_entry, 6, 1)
        ...
        self.failure_type_dropdown.addItems(["Perforation", "Detached spall", "Incipient spall"])
    ...
    def on_material1_selected(self, index):
        selected_material = self.material0_dropdown.itemText(index)
        self.density1_entry.setText(str(self.df_materials.loc[self.df_materials['mat'] == selected_material, 'density'].values[0]))
        self.hardness_entry.setText(str(self.df_materials.loc[self.df_materials['mat'] == selected_material, 'hardness'].values[0]))
        soundspeed = self.packages['np'].sqrt(self.df_materials.loc[self.df_materials['mat'] == selected_material, 'tensile_modulus'].values[0]/self.df_materials.loc[self.df_materials['mat'] == selected_material, 'density'].values[0])
        self.soundspeed_entry.setText(f'{soundspeed:.2f}')           
    ...
    def on_target_type_selected(self, index):
        ...
        if selected_material == "Aluminium":
            self.density1_entry.setText(str(self.df_materials.loc[self.df_materials['mat'] == self.initial_target_mat, 'density'].values[0]))
            self.hardness_entry.setText(str(self.df_materials.loc[self.df_materials['mat'] == self.initial_target_mat, 'hardness'].values[0]))
            soundspeed = self.packages['np'].sqrt(self.df_materials.loc[self.df_materials['mat'] == self.initial_target_mat, 'tensile_modulus'].values[0]/self.df_materials.loc[self.df_materials['mat'] == self.initial_target_mat, 'density'].values[0])
            self.soundspeed_entry.setText(f'{soundspeed:.2f}')           
            self.hardness_entry.setDisabled(False)
            self.soundspeed_entry.setDisabled(False)
            self.material1_dropdown.setDisabled(False)
            self.material1_dropdown.setCurrentText(self.initial_target_mat)
        elif selected_material == "Titanium":
            self.density1_entry.setText("")
            self.hardness_entry.setText("")
            self.soundspeed_entry.setText("")
            self.hardness_entry.setDisabled(False)
            self.soundspeed_entry.setDisabled(False)
            self.material1_dropdown.setDisabled(True)
            self.material1_dropdown.setCurrentText("")            
        else:
            self.density1_entry.setText("")    
            self.hardness_entry.setText("")
            self.soundspeed_entry.setText("")
            self.hardness_entry.setDisabled(True)   
            self.soundspeed_entry.setDisabled(True)   
            self.material1_dropdown.setDisabled(True)
            self.material1_dropdown.setCurrentText("")                                                        
    ...
    def on_run_button_clicked(self):
        ...
        try:
            # For steel, CFRP, and fibreglass targets fill the missing fields with '0'
            if self.target_type_dropdown.currentText() == "Steel" or self.target_type_dropdown.currentText() == "CFRP" or self.target_type_dropdown.currentText() == "Fibreglass":
                self.hardness_entry.setText("0")
                self.soundspeed_entry.setText("0")
            
            # Check if any of the text boxes are empty
            if not all([self.angle_entry.text(), self.density0_entry.text(),self.thickness_entry.text(), 
                        self.density1_entry.text(), self.hardness_entry.text(),self.soundspeed_entry.text()]):
            ...
            data = {
                ...
                'angle': float(self.angle_entry.text()),  # units = degrees
                'type': self.target_type_dropdown.currentText(), 
                'target_mat': self.material1_dropdown.currentText(), 
                'shield_thick': float(self.thickness_entry.text()),  # units = cm
                'shield_density': float(self.density1_entry.text()),  # units = g/cm3           
                'shield_HB': float(self.hardness_entry.text()),  # units = HB
                'shield_C': float(self.soundspeed_entry.text()),  # units = m/s
                'MLI_AD': float(self.MLI_entry.text()) if self.MLI_entry.text != "" else 0  # units = g/cm2
            }

            ## add material type code
            if self.target_type_dropdown.currentText() == "Aluminium":
                data['type'] = 'AL' 
            elif self.target_type_dropdown.currentText() == "Steel":                
                data['type'] = 'ST'
            elif self.target_type_dropdown.currentText() == "Titanium":                
                data['type'] = 'TI'
            elif self.target_type_dropdown.currentText() == "CFRP":                
                data['type'] = 'CFRP'                
            elif self.target_type_dropdown.currentText() == "Fibreglass":                
                data['type'] = 'fibreglass'       

            ## add analysis type code
            if self.failure_type_dropdown.currentText() == "Perforation":
                data['mode'] = 'perforate' 
            elif self.failure_type_dropdown.currentText() == "Detached spall":                
                data['mode'] = 'detached_spall'
            elif self.failure_type_dropdown.currentText() == "Incipient spall":
                data['mode'] = 'incipient_spall'
            ...
            df_plot['dc_BLE'] = df_plot.apply(self.packages['singleWall_performance'],axis=1)

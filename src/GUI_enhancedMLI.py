...
class PackageLoader(QThread):
    ...
    def run(self):
        ...
        self.packages_loaded.emit({
            ...
            'mli_performance': mli_performance
        })
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        self.density1_label = QLabel("Wall density (g/cm³)):", self)
        ...
        # Add a note at the bottom of the frame
        self.note_label = QLabel("NOTE: specify wall thickness OR areal density", self)
        self.note_label.setStyleSheet("font-size: 10px;")  # Set the font size to 10px
    ...
    def on_run_button_clicked(self):
        ...
        try:
            ...
            if self.wallAD_entry.text() == "":
                self.wallAD_entry.setText("{:.2f}".format(float(self.wallThickness_entry.text())*float(self.density1_entry.text())))
            elif self.wallThickness_entry.text() == "":
                self.wallThickness_entry.setText("{:.2f}".format(float(self.wallAD_entry.text()) / float(self.density1_entry.text())))
            elif self.wallAD_entry.text() != "" and self.density1_entry.text() != "":
                raise ValueError("Enter either wall thickness or AD.")
            ...
            data = {
                ...
                'wall_thick': float(self.wallThickness_entry.text()),  #  units = cm
                'wall_density': float(self.density1_entry.text()),  # units = g/cm3        
                'bumper_AD': float(self.bumperAD_entry.text()),  # units = g/cm2
                'wall_AD': float(self.wallAD_entry.text()),  # units = g/cm2
                'MLI_AD': float(self.MLIad_entry.text()),  # units = g/cm2
                'thickness': float(self.totalThickness_entry.text())  #  units = cm
            }
            ...
            df_plot['dc_BLE'] = df_plot.apply(self.packages['mli_performance'],axis=1)
